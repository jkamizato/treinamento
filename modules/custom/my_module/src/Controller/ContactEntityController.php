...
class ContactEntityController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionPageTitle($contact_entity_revision) {
    $contact_entity = $this->entityManager()->getStorage('contact_entity')->loadRevision($contact_entity_revision);
    return $this->t('Revision of %title from %date', array('%title' => $contact_entity->label(), '%date' => format_date($contact_entity->getRevisionCreationTime())));
  }

  /**
   * Generates an overview table of older revisions of a Contact entity .
   *
   * @param \Drupal\my_module\Entity\ContactEntityInterface $contact_entity
  ...
  public function revisionOverview(ContactEntityInterface $contact_entity) {
    ...
    foreach (array_reverse($vids) as $vid) {
      /** @var \Drupal\my_module\ContactEntityInterface $revision */
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              ...
              Url::fromRoute('contact_entity.revision_revert_translation_confirm', ['contact_entity' => $contact_entity->id(), 'contact_entity_revision' => $vid, 'langcode' => $langcode]) :
              Url::fromRoute('contact_entity.revision_revert_confirm', ['contact_entity' => $contact_entity->id(), 'contact_entity_revision' => $vid]),
            ];
          }
          ...
          if ($delete_permission) {
            $links['delete'] = [
              'title' => $this->t('Delete'),
              'url' => Url::fromRoute('contact_entity.revision_delete_confirm', ['contact_entity' => $contact_entity->id(), 'contact_entity_revision' => $vid]),
            ];
          }
    ...
    $build['contact_entity_revisions_table'] = array(
      '#theme' => 'table',
      '#rows' => $rows,
      '#header' => $header,
    );
